...
export const BarrelCard: React.FC<BarrelCardProps> = ({ barrel, onConsume, onRefill, onDelete }) => {
  ...
  return (
    <div className="card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.25rem', padding: '1.5rem 1.25rem', position: 'relative' }}>
      {onDelete && (
        <button 
          onClick={onDelete}
          style={{ position: 'absolute', top: '0.75rem', right: '0.75rem', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: '0.25rem' }}
